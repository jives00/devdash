    ...
    <div className="bg-gray-900 border border-gray-700/50 rounded-xl p-5 flex flex-col gap-4">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          {totalErrors > 0 && (
            <span className="bg-red-900/60 text-red-300 border border-red-700/40 text-sm font-bold px-2 py-0.5 rounded-full">
      ...
      {project.healthUrl && health && (
        <div className={`text-sm flex flex-wrap items-center gap-2 ${health.ok ? 'text-green-400' : 'text-red-400'}`}>
          <span className="font-medium">{health.ok ? '✓ Healthy' : '✗ Unhealthy'}</span>
          {health.latencyMs > 0 && <span className="text-gray-400">{health.latencyMs}ms</span>}
          {health.db && <span className="text-gray-400">DB: {health.db}</span>}
          {health.error && <span className="text-red-300 break-all">{health.error}</span>}
        </div>
      )}
        ...
        {project.containers.map(c => (
          <div key={c.name} className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-gray-200 font-mono text-sm">{c.name}</span>
            </div>
            <div className="flex items-center gap-3 flex-shrink-0">
              {c.info?.uptime && <span className="text-gray-400 text-sm">up {c.info.uptime}</span>}
              {(c.info?.restartCount ?? 0) > 0 && (
                <span className="text-yellow-400 text-sm">{c.info!.restartCount} restart{c.info!.restartCount !== 1 ? 's' : ''}</span>
              )}
              <Link href={`/logs/${c.name}`} className="text-sm text-blue-400 hover:text-blue-300">
                Logs
              </Link>
            </div>
          </div>
      ...
      {project.oneShot.map(s => (
        <div key={s.container} className="text-sm text-gray-400 flex items-center gap-2">
          <span>{s.name}:</span>
          {s.finishedAt && s.finishedAt !== '0001-01-01T00:00:00Z' ? (
            <>
              ...
              <span className="text-gray-400">{formatRelative(s.finishedAt)}</span>
      ...
      {/* Last error — full text, no truncation */}
      ...
      <div>
        ...
        {expanded && project.containers.slice(0, 1).map(c => (
          <div key={c.name} className="bg-black/40 rounded p-2 font-mono text-sm text-gray-300 space-y-0.5 max-h-36 overflow-y-auto">
            {c.logPreview.length > 0
              ? c.logPreview.map((line, i) => <div key={i} className="break-all">{line}</div>)
